class UserController {
  ...
  getGender(name) {
    ...
    const genders = global.config.cinsiyetler.Names.filter((entry) =>
  ...
  isValidName(name) {
    return global.config.cinsiyetler.Names.some(
  ...
  async getUser(req, res) {
    ...
    try {
      ...
      if (user) {
        ...
        if (
          !global.config.cinsiyetler.Names.some((x) => x.name.includes(topName))
        ) {
          let mostUsedGenderName = "";
          let mostUsedGenderCount = 0;
          global.config.cinsiyetler.Names.forEach((gender) => {
            if (
